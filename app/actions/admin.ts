...
export async function getAdminKPIs() {
  ...
  const avgInnovationScore =
    profiles
      ...
      .reduce((sum, p) => sum + (p.innovation_score ?? 0), 0) / studentCount || 0
  ...
  const domainBreakdown = startups?.reduce(
    (acc, s) => {
...
export async function getEcosystemCharts() {
  ...
  const stageDistribution = startups?.reduce(
    (acc, s) => {
      const stage = s.stage ?? 'unknown'
      acc[stage] = (acc[stage] ?? 0) + 1
      return acc
  ...
  const connectionsByDomain = mentorships?.reduce(
    (acc, m) => {
...
export async function getUserGrowthMetrics() {
  ...
  // Group by month
  const monthlyGrowth: Record<string, { students: number; mentors: number; investors: number }> = {}
